...
const CACHE_URLS = [
  "/jardinagem/manifest.json",
  "/static/icons/logo-garra.jpg",
...
self.addEventListener("fetch", event => {
  ...
  // Outras rotas de API — Network First, sem cache
...
function abrirDB() {
  return new Promise((res, rej) => {
    ...
    req.onupgradeneeded = e => {
      ...
      // Store de pares (criados offline)
      ...
      // Store de fotos da bancada (offline)
...
async function processarFilaCompleta() {
  ...
  try {
    ...
    console.error("[SW] Erro ao abrir DB:", e);
    return resultado;
  }

  // 1. Pares pendentes → cria no banco primeiro
  const paresPend = await getAll(db, STORE_PARES, "enviado", false).catch(() => []);
  const parMap = {}; // offline_id → db_id

  for (const par of paresPend) {
    try {
      const resp = await fetch("/api/pares", {
        method:  "POST",
        headers: {
        ...
        body: JSON.stringify({
          semana_id:  par.semana_id,
          local_nome: par.local_nome || "",
          data_label: "",
          ordem:      99,
        }),
      });
  ...
  // 2. Fotos pendentes → envia para Supabase via backend
  const fotosPend = await getAll(db, STORE_FOTOS, "enviado", false).catch(() => []);

  for (const item of fotosPend) {
    try {
      // Resolve par_id: pode ser db_id já conhecido ou precisar do mapa
      let par_id = item.par_db_id || parMap[item.par_ref] || null;

      // Se não tem par ainda, cria agora
      if (!par_id && item.semana_id) {
        const rp = await fetch("/api/pares", {
          method:  "POST",
          headers: {
            "Content-Type":  "application/json",
      ...
      const resp = await fetch("/api/fotos/avulsa", {
        method:  "POST",
      ...
      if (resp.ok) {
        await marcarEnviado(db, STORE_FOTOS, item.offline_id);
        resultado.fotos++;
      } else {
        resultado.erros++;
      }
      ...
      resultado.erros++;
      console.warn("[SW] Erro foto:", item.offline_id, e);
    }
  }
  ...
  for (const item of kmPend) {
    try {
      const resp = await fetch("/api/relatorios/km", {
        method:  "POST",
        headers: {
          "Content-Type":  "application/json",
          "Authorization": `Bearer ${item.token}`,
      ...
      if (resp.ok) {
        await marcarEnviado(db, STORE_KM, item.offline_id);
        resultado.km++;
      } else {
        resultado.erros++;
      }
